...
export function ProfileTabs({ activeTab, onSelectTab, boutsCount }: ProfileTabsProps) {
  return (
    ...
    >
      <div role="tablist" aria-label="Fighter profile sections" className="flex h-11 rounded-xl border border-slate-200/80 bg-slate-100/90 p-1 shadow-xs dark:border-slate-800 dark:bg-slate-900">
        <button
          type="button"
          role="tab"
          aria-selected={activeTab === 'profile'}
          onClick={() => onSelectTab('profile')}
          className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg text-[12px] sm:text-[13px] font-semibold transition ${
            activeTab === 'profile'
        ...
        >
          ...
          <span
            className={`rounded-full px-1.5 py-0.2 font-sans text-[12px] font-extrabold ${
              activeTab === 'history'
