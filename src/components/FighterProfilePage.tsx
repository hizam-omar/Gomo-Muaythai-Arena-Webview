...
import { ArrowLeft, Check, LockKeyhole, Pencil, Save, ShieldCheck, X } from 'lucide-react';
...
type EditableFighter = Required<Pick<Fighter,
  ...
  'ifmaLicense' | 'stance' | 'favTechnique'>>;
...
function editableValues(fighter: Fighter): EditableFighter {
  return {
    ...
    favTechnique: fighter.favTechnique || 'Knee Strike',
  };
}
...
export function FighterProfilePage({ fighter, isLoading, theme, onToggleTheme }: FighterProfilePageProps) {
  ...
  const avatar = fighter ? publicAvatar(fighter) : undefined;
  ...
  const unlock = () => {
    const expected = (fighter?.nokp || '').replace(/\D/g, '').slice(0, 6);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/95">
        <div className="mx-auto flex max-w-4xl items-center gap-2 px-3 py-2 sm:px-4">
          ...
          <button type="button" onClick={onToggleTheme} className="rounded-lg border border-slate-200 px-2.5 py-2 text-[10px] font-black uppercase dark:border-slate-700">{theme === 'light' ? 'Dark' : 'Light'}</button>
      ...
      <main className="mx-auto max-w-4xl space-y-4 px-3 py-4 sm:px-4 sm:py-6">
        ...
        <Section eyebrow="Fighter profile" title="Personal & Physical Specs">
          <Field label="Full Name" value={form.name} editing={editing} onChange={setText('name')} /><Field label="Nickname" value={form.nickname} editing={editing} onChange={setText('nickname')} /><Field label="MyKad / NO KP" value={editing ? form.nokp : MASK} editing={editing} onChange={setText('nokp')} /><Field label="IFMA License" value={form.ifmaLicense} editing={editing} onChange={setText('ifmaLicense')} /><Field label="Date of Birth" value={editing ? form.dob : MASK} editing={editing} onChange={setText('dob')} /><Field label="Age" value={form.age} editing={editing} type="number" onChange={setNumber('age')} /><Field label="Weight (kg)" value={form.weightKg} editing={editing} type="number" onChange={setNumber('weightKg')} /><Field label="Height (cm)" value={form.heightCm} editing={editing} type="number" onChange={setNumber('heightCm')} /><Field label="Fighting Stance" value={form.stance} editing={editing} onChange={setText('stance')} options={['Orthodox', 'Southpaw', 'Switch']} /><Field label="Favorite Strike" value={form.favTechnique} editing={editing} onChange={setText('favTechnique')} /><Field label="Wins" value={form.wins} editing={editing} type="number" onChange={setNumber('wins')} /><Field label="Losses" value={form.losses} editing={editing} type="number" onChange={setNumber('losses')} /><Field label="Draws" value={form.draws} editing={editing} type="number" onChange={setNumber('draws')} /><Field label="Record Win Rate" value={`${winRate}%`} editing={false} />
        </Section>
        <Section eyebrow="Academic info" title="School & Academic Specs"><Field label="School" value={form.school} editing={editing} onChange={setText('school')} /><Field label="Form / Class" value={form.gradeClass} editing={editing} onChange={setText('gradeClass')} /><Field label="Class Teacher" value={form.classTeacher} editing={editing} onChange={setText('classTeacher')} /><Field label="Senior Assistant (Academic)" value={form.pkTeacher} editing={editing} onChange={setText('pkTeacher')} /></Section>
        <Section eyebrow="Parent / guardian info" title="Parent & Emergency Contact"><Field label="Parent Name" value={form.parentName} editing={editing} onChange={setText('parentName')} /><Field label="Parent H/P No." value={editing ? form.parentPhone : MASK} editing={editing} onChange={setText('parentPhone')} /><Field label="Parent Email" value={editing ? form.parentEmail : MASK} editing={editing} type="email" onChange={setText('parentEmail')} /></Section>
        <Section eyebrow="Official club" title="Club & Management"><Field label="Club" value={form.club} editing={editing} onChange={setText('club')} /><Field label="Manager" value={form.manager} editing={editing} onChange={setText('manager')} /></Section>
        <p className="pb-5 text-center text-[11px] font-semibold text-slate-400">Last updated: {updatedLabel}</p>
      </main>

      {unlockOpen && <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/75 p-4" role="dialog" aria-modal="true" aria-labelledby="unlock-title" onClick={() => setUnlockOpen(false)}><div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl dark:bg-slate-900" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div className="grid h-11 w-11 place-items-center rounded-xl bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300"><LockKeyhole className="h-5 w-5" /></div><button type="button" onClick={() => setUnlockOpen(false)} aria-label="Close unlock dialog" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"><X className="h-4 w-4" /></button></div><h2 id="unlock-title" className="mt-4 text-xl font-black">Unlock profile editing</h2><p className="mt-1 text-sm text-slate-500">Enter the first 6 digits of this fighter’s MyKad number.</p><label className="mt-4 block text-[11px] font-black uppercase tracking-wide text-slate-500" htmlFor="mykad-pin">6-digit MyKad code</label><input id="mykad-pin" data-testid="mykad-pin-input" autoFocus inputMode="numeric" maxLength={6} value={pin} onChange={(event) => { setPin(event.target.value.replace(/\D/g, '').slice(0, 6)); setUnlockError(''); }} onKeyDown={(event) => { if (event.key === 'Enter') unlock(); }} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-center font-mono text-2xl font-black tracking-[0.35em] outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100 dark:border-slate-700 dark:bg-slate-950 dark:focus:ring-red-950" placeholder="••••••" />{unlockError && <p role="alert" className="mt-2 text-xs font-bold text-red-600">{unlockError}</p>}<button type="button" data-testid="unlock-profile-button" onClick={unlock} className="mt-4 w-full rounded-xl bg-red-600 px-4 py-3 text-sm font-black text-white hover:bg-red-700">Verify & edit profile</button></div></div>}
    </div>
  );
}
